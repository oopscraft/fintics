...
public abstract class KrxBrokerClient extends BrokerClient {

    public KrxBrokerClient(BrokerClientDefinition definition, Properties config) {
        super(definition, config);
    }

    @Override
    public List<BrokerAsset> getBrokerAssets() {
        List<BrokerAsset> brokerAssets = new ArrayList<>();
    ...
    private List<BrokerAsset> getStockBrokerAssetByMarketType(String marketType) {
    ...
    protected List<BrokerAsset> getEtfBrokerAssets() {
        RestTemplate restTemplate = RestTemplateBuilder.create()
                .insecure(true)
                .readTimeout(30_000)
                .build();
